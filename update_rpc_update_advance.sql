CREATE OR REPLACE FUNCTION public.rpc_update_advance(p_advance_id uuid, p_amount numeric, p_account_id uuid, p_description text, p_advance_date date, p_recipient_type text DEFAULT NULL::text, p_recipient_id uuid DEFAULT NULL::uuid)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
BEGIN
  ...
  IF v_entry_id IS NOT NULL THEN
    ...
    SET total_amount = p_amount,
        remaining_amount = p_amount - ABS(paid_amount),
