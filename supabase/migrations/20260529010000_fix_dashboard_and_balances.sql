-- Migration to fix dashboard history and recalculate balances
SET search_path = public;

-- 1. Update the fn_update_account_balance trigger function
...
BEGIN
    ...
    SELECT COALESCE(SUM(
        CASE 
            WHEN lower(type) IN ('in', 'credit') THEN amount 
            WHEN lower(type) IN ('out', 'debit') THEN -amount 
            ELSE 0 
        END
...
-- 2. Recalculate all account balances using the new logic
DO $$
DECLARE
    r_account RECORD;
    v_initial_balance NUMERIC(15, 2) := 0;
    ...
    v_new_balance NUMERIC(15, 2) := 0;
BEGIN
    FOR r_account IN SELECT id, company_id FROM accounts LOOP
        -- Fetch initial balance and its date
        v_initial_balance := 0;
        v_initial_balance_date := NULL;
        
        SELECT amount, balance_date INTO v_initial_balance, v_initial_balance_date
        FROM initial_balances
        WHERE account_id = r_account.id AND company_id = r_account.company_id
        LIMIT 1;

        -- Fetch sum of valid transactions
        SELECT COALESCE(SUM(
            CASE 
                WHEN lower(type) IN ('in', 'credit') THEN amount 
                WHEN lower(type) IN ('out', 'debit') THEN -amount 
        ...
        SET current_balance = v_new_balance,
            balance = v_new_balance,
            updated_at = NOW()
        WHERE id = r_account.id AND company_id = r_account.company_id;
    END LOOP;
END;
$$;

-- 3. Fix the rpc_dashboard_data bank_history calculation
...
BEGIN
  ...
  -- 1. Saldo Disponível (Apenas contas reais)
  ...
  -- 5. Empréstimos Concedidos e Tomados
  ...
  -- 6. Adiantamentos Concedidos e Tomados
  SELECT 
  ...
  -- 10. Histórico de Evolução do Patrimônio Líquido
  IF v_loading_source IS NOT NULL THEN
    EXECUTE format($dyn$
      ...
      -- NEW bank_history: Calculates balances considering initial_balance_date properly
      bank_history AS (
        SELECT mg.i, 
               COALESCE((
                 SELECT SUM(
                   COALESCE((SELECT SUM(ib.amount) FROM initial_balances ib WHERE ib.account_id = a.id AND ib.company_id = $1 AND ib.balance_date <= mg.month_end), 0) + 
                   COALESCE((
                     SELECT SUM(CASE WHEN lower(ft.type) IN ('in', 'credit') THEN ft.amount WHEN lower(ft.type) IN ('out', 'debit') THEN -ft.amount ELSE 0 END) 
                     FROM financial_transactions ft 
                     WHERE ft.account_id = a.id 
                       AND ft.company_id = $1 
                       AND ft.transaction_date <= mg.month_end 
                       AND ft.status != 'canceled'
                       AND (NOT EXISTS (SELECT 1 FROM initial_balances ib2 WHERE ib2.account_id = a.id AND ib2.company_id = $1) 
                            OR ft.transaction_date >= (SELECT ib3.balance_date FROM initial_balances ib3 WHERE ib3.account_id = a.id AND ib3.company_id = $1 LIMIT 1))
                   ), 0)
                 ) 
                 FROM accounts a WHERE a.company_id = $1 AND a.is_active = true
               ), 0) AS bank_assets 
        FROM month_grid mg
      ),
