-- ============================================================================
-- MIGRATION: 20260525191500_fix_dashboard_cancelled_entries_in_history.sql
-- Objetivo: Corrigir o cálculo do histórico de patrimônio líquido no dashboard,
--           garantindo que transações vinculadas a contas a pagar ou a receber
--           que foram canceladas/estornadas sejam ignoradas e que estornos/reversões
--           sejam devidamente deduzidos usando a direção da transação.
-- ============================================================================
  ...
  IF v_loading_source IS NOT NULL THEN
    EXECUTE format($dyn$
      ...
      receivables_history AS (
        SELECT mg.i, 
               COALESCE((SELECT SUM(fe.total_amount) FROM financial_entries fe WHERE fe.company_id = $1 AND fe.type = 'receivable' AND COALESCE(fe.status, 'open') NOT IN ('canceled', 'cancelled') AND fe.created_date <= mg.month_end), 0) - 
               COALESCE((SELECT SUM(
                 CASE WHEN lower(ft.type) IN ('in', 'credit') THEN ft.amount 
                      WHEN lower(ft.type) IN ('out', 'debit') THEN -ft.amount 
                      ELSE 0 END
               ) FROM financial_transactions ft JOIN financial_entries fe ON fe.id = ft.entry_id WHERE fe.company_id = $1 AND fe.type = 'receivable' AND COALESCE(fe.status, 'open') NOT IN ('canceled', 'cancelled') AND ft.transaction_date <= mg.month_end), 0) AS pending_receivables 
        FROM month_grid mg
      ),
      payables_history AS (
        SELECT mg.i, 
               COALESCE((SELECT SUM(fe.total_amount) FROM financial_entries fe WHERE fe.company_id = $1 AND fe.type = 'payable' AND COALESCE(fe.status, 'open') NOT IN ('canceled', 'cancelled') AND fe.created_date <= mg.month_end), 0) - 
               COALESCE((SELECT SUM(
                 CASE WHEN lower(ft.type) IN ('out', 'debit') THEN ft.amount 
                      WHEN lower(ft.type) IN ('in', 'credit') THEN -ft.amount 
                      ELSE 0 END
               ) FROM financial_transactions ft JOIN financial_entries fe ON fe.id = ft.entry_id WHERE fe.company_id = $1 AND fe.type = 'payable' AND COALESCE(fe.status, 'open') NOT IN ('canceled', 'cancelled') AND ft.transaction_date <= mg.month_end), 0) AS pending_payables 
        FROM month_grid mg
      ),
