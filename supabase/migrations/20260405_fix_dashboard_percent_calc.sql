-- Fix rpc_dashboard_data to handle growth calculation from zero balance
-- Date: 2026-04-05
...
BEGIN
    ...
    WHERE company_id = p_company_id AND type = 'payable' AND LOWER(status) IN ('pending', 'partial', 'open');

    SELECT COALESCE(SUM(remaining_amount), 0) INTO v_total_receivables
    FROM public.financial_entries
    WHERE company_id = p_company_id AND type = 'receivable' AND LOWER(status) IN ('pending', 'partial', 'open');
    ...
    -- Merchandise in Transit (Estoque em Trânsito Hoje)
    ...
    -- 2. OPERATIONAL KPIs (Last 30 Days)
    ...
    SELECT json_agg(json_build_object(
        'name', to_char(mg.month_start, 'Mon/YY'),
    ...
    -- 4. HISTÓRICO PATRIMONIAL (Snapshot)
    WITH months_6 AS (
    ...
    snapshots AS (
        SELECT 
            m6.month_start,
            to_char(m6.month_start, 'Mon/YY') as name,
            COALESCE((SELECT SUM(amount) FROM financial_transactions WHERE company_id = p_company_id AND type = 'credit' AND date_trunc('month', transaction_date) = m6.month_start), 0) as recebido,
            ...
            COALESCE((SELECT SUM(amount) FROM financial_transactions WHERE company_id = p_company_id AND type = 'debit' AND date_trunc('month', transaction_date) = m6.month_start), 0) as pago,
            ...
            -- Net Worth snapshot
            (
                (v_total_bank - COALESCE((SELECT SUM(CASE WHEN type='credit' THEN amount ELSE -amount END) FROM financial_transactions WHERE company_id = p_company_id AND transaction_date > m6.month_end), 0))
                + COALESCE((SELECT SUM(total_amount - COALESCE((SELECT SUM(amount) FROM financial_transactions ft WHERE ft.entry_id = fe.id AND ft.transaction_date <= m6.month_end), 0)) 
    ...
    history_with_growth AS (
        SELECT 
            *,
            -- Handle division by zero for monthly change
            CASE 
    ...
    -- Growth percent (period)
    -- Handle zero-balance baselines and dynamic array length
    SELECT 
        CASE 
    ...
    SELECT json_build_object(
        ...
        'financialPending', json_build_object(
            'receivables', COALESCE((SELECT json_agg(t) FROM (SELECT * FROM financial_entries WHERE company_id = p_company_id AND type = 'receivable' AND LOWER(status) != 'paid' ORDER BY due_date ASC LIMIT 5) t), '[]'),
            'tradePayables', COALESCE((SELECT json_agg(t) FROM (SELECT * FROM financial_entries WHERE company_id = p_company_id AND type = 'payable' AND LOWER(status) != 'paid' ORDER BY due_date ASC LIMIT 5) t), '[]'),
            'expenses', '[]'::json
        ),
