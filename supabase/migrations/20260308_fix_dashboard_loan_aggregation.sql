-- Migration: fix_dashboard_loan_aggregation
-- Description: Updates rpc_dashboard_data to include loans from the new system and fix Net Worth calculation.
...
BEGIN
  ...
  -- but net_worth must subtract EVERYTHING.
  v_total_liabilities := 
    v_pending_payables + 
    v_loans_taken + 
  ...
  -- Para o histórico, vamos manter o logic existente mas garantir que net_worth use os novos termos se disponíveis
  IF v_loading_source IS NOT NULL THEN
    EXECUTE format($dyn$
      WITH months AS (
      ...
      receivables_history AS (
        SELECT mg.i, COALESCE((SELECT SUM(l.total_sales_value) FROM %s l WHERE l.company_id = $1 AND l.status NOT IN ('canceled', 'cancelled') AND l.unload_weight_kg IS NOT NULL AND l.loading_date <= mg.month_end), 0)
          - COALESCE((SELECT SUM(ft.amount) FROM financial_transactions ft JOIN financial_entries fe ON fe.id = ft.entry_id WHERE fe.company_id = $1 AND fe.type LIKE '%%%%receivable%%%%' AND ft.transaction_date <= mg.month_end), 0) AS pending_receivables
        FROM month_grid mg
      ),
      payables_history AS (
        SELECT mg.i, COALESCE((SELECT SUM(l.total_purchase_value + l.total_freight_value) FROM %s l WHERE l.company_id = $1 AND l.status NOT IN ('canceled', 'cancelled') AND l.loading_date <= mg.month_end), 0)
          - COALESCE((SELECT SUM(ft.amount) FROM financial_transactions ft JOIN financial_entries fe ON fe.id = ft.entry_id WHERE fe.company_id = $1 AND fe.type LIKE '%%%%payable%%%%' AND ft.transaction_date <= mg.month_end), 0) AS pending_payables
        FROM month_grid mg
      ),
      transit_history AS (
      ...
      hist AS (
        SELECT
          ...
          ph.pending_payables AS liabilities,
          (bh.bank_assets + rh.pending_receivables + th.transit_value + ah.fixed_assets - ph.pending_payables) AS net_worth
        FROM month_grid mg
