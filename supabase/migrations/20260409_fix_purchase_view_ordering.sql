-- [FIX] Step 1: Add missing commission_id column to financial_links
DO $$ 
BEGIN 
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='financial_links' AND column_name='commission_id') THEN
        ALTER TABLE public.financial_links ADD COLUMN commission_id UUID REFERENCES public.ops_purchase_order_commissions(id) ON DELETE SET NULL;
        CREATE INDEX idx_financial_links_commission ON public.financial_links(commission_id);
    END IF;
END $$;

-- [FIX] Step 2: Redefine the enriched view with corrected logic
 ...
 WITH order_loadings AS (
         ...
         broker_metadata AS (
           SELECT 
             ...
             commission_per_sc,
             broker_id,
             (broker_id IS NOT NULL) as has_broker -- ✅ CORREÇÃO: Derivado do broker_id
  ...
  SELECT po.id,
     ...
     po.company_id,
     po.created_at, -- ✅ Para ordenação no frontend
     po.metadata
