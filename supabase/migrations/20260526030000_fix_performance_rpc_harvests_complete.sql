-- ============================================================================
-- Migration: Fix rpc_performance_report — restore complete harvests fields
-- and treat empty UF as 'N/I'
-- Data: 2026-05-26
-- ============================================================================
-- CORRIGE:
--   1) Harvests: restores all fields (volumeSc, avgPurchasePrice, avgSalesPrice,
--      avgFreightPrice, totalPurchase, totalFreight) that were lost in a
--      previous simplified deployment
--   2) Harvests: treats empty/null loadingState as 'N/I' (Não Informado)
--   3) Restores priceTrendHistory, expenseBreakdown, topProfitOrders,
--      topLossOrders, bestMonths, worstMonths to the result JSON
-- ============================================================================
...
BEGIN
  ...
  -- 4. HISTÓRICO MENSAL
  -- ========================================================================
  WITH months AS (SELECT generate_series(0, v_months - 1) AS i),
  month_grid AS (
    SELECT i, date_trunc('month', (CURRENT_DATE - (i || ' months')::interval))::date AS month_start,
  ...
  loading_monthly AS (
    SELECT date_trunc('month', l.loading_date)::date AS month_start,
    ...
    FROM ops_loadings l
    WHERE l.company_id = p_company_id AND l.status NOT IN ('canceled', 'cancelled') AND l.loading_date >= v_date_from
    GROUP BY 1
  ),
  expense_monthly AS (
    SELECT date_trunc('month', COALESCE(ae.expense_date, ae.due_date, CURRENT_DATE))::date AS month_start,
      COALESCE(SUM(ae.amount), 0) AS other_expenses
    FROM admin_expenses ae
    WHERE ae.company_id = p_company_id AND ae.status != 'cancelled' AND COALESCE(ae.expense_date, ae.due_date, CURRENT_DATE) >= v_date_from
    GROUP BY 1
  )
  SELECT json_agg(
    json_build_object(
      'name', mg.month_name, 'fullDate', mg.full_date,
      ...
      'netResult', COALESCE(lm.revenue, 0) - COALESCE(lm.purchase_cost, 0) - COALESCE(lm.freight_cost, 0) - COALESCE(em.other_expenses, 0),
      'totalQuantitySc', CASE WHEN COALESCE(lm.total_kg, 0) > 0 THEN ROUND(lm.total_kg / 60, 2) ELSE 0 END,
      'avgPurchaseCostSc', CASE WHEN COALESCE(lm.total_kg, 0) > 0 THEN ROUND(lm.purchase_cost / (lm.total_kg / 60), 2) ELSE 0 END,
      'avgFreightCostSc', CASE WHEN COALESCE(lm.total_kg, 0) > 0 THEN ROUND(lm.freight_cost / (lm.total_kg / 60), 2) ELSE 0 END,
      'avgOtherCostSc', CASE WHEN COALESCE(lm.total_kg, 0) > 0 THEN ROUND(COALESCE(em.other_expenses, 0) / (lm.total_kg / 60), 2) ELSE 0 END,
      'avgTotalCostSc', CASE WHEN COALESCE(lm.total_kg, 0) > 0 THEN ROUND((COALESCE(lm.purchase_cost, 0) + COALESCE(lm.freight_cost, 0) + COALESCE(em.other_expenses, 0)) / (lm.total_kg / 60), 2) ELSE 0 END
    ) ORDER BY mg.i DESC
  ...
  price_monthly AS (
    SELECT date_trunc('month', l.loading_date)::date AS month_start,
      CASE WHEN SUM(l.weight_kg) > 0 THEN ROUND(SUM(l.total_purchase_value) / (SUM(l.weight_kg) / 60), 2) ELSE 0 END AS avg_purchase,
      CASE WHEN SUM(l.weight_kg) > 0 THEN ROUND(SUM(l.total_sales_value) / (SUM(l.weight_kg) / 60), 2) ELSE 0 END AS avg_sales
    FROM ops_loadings l
    WHERE l.company_id = p_company_id AND l.status NOT IN ('canceled', 'cancelled') AND l.loading_date >= v_date_from
    GROUP BY 1
  )
  SELECT json_agg(
    json_build_object('name', mg.month_name, 'avgPurchasePrice', COALESCE(pm.avg_purchase, 0), 'avgSalesPrice', COALESCE(pm.avg_sales, 0))
    ORDER BY mg.i DESC
  ) INTO v_price_trend
  FROM month_grid mg LEFT JOIN price_monthly pm ON pm.month_start = mg.month_start;

  -- ========================================================================
  -- 6. HARVEST BREAKDOWN (por UF) — trata UF vazio como 'N/I'
  -- ========================================================================
  SELECT COALESCE(json_agg(row_to_json(h)), '[]'::json)
  INTO v_harvests
  FROM (
    SELECT
      CASE WHEN COALESCE(NULLIF(TRIM(po.metadata->>'loadingState'), ''), '') = ''
           THEN 'N/I' ELSE UPPER(TRIM(po.metadata->>'loadingState')) END AS uf,
      ROUND(SUM(l.weight_kg) / 1000, 2) AS "volumeTon",
      ROUND(SUM(l.weight_kg) / 60, 2) AS "volumeSc",
      CASE WHEN SUM(l.weight_kg) > 0 THEN ROUND(SUM(l.total_purchase_value) / (SUM(l.weight_kg) / 60), 2) ELSE 0 END AS "avgPurchasePrice",
      CASE WHEN SUM(l.weight_kg) > 0 THEN ROUND(SUM(l.total_sales_value) / (SUM(l.weight_kg) / 60), 2) ELSE 0 END AS "avgSalesPrice",
      CASE WHEN SUM(l.weight_kg) > 0 THEN ROUND(SUM(l.total_freight_value) / (SUM(l.weight_kg) / 1000), 2) ELSE 0 END AS "avgFreightPrice",
  ...
  WITH expense_cats AS (
    SELECT COALESCE(ec.name, 'Outros') AS cat_name, COALESCE(ec.type, 'variable') AS cat_type, SUM(ae.amount) AS total
    FROM admin_expenses ae LEFT JOIN expense_categories ec ON ec.id = ae.category_id
    WHERE ae.company_id = p_company_id AND ae.status != 'cancelled' AND COALESCE(ae.expense_date, ae.due_date, CURRENT_DATE) >= v_date_from
    GROUP BY ec.name, ec.type
  ),
  grouped AS (
    SELECT
      CASE WHEN cat_type = 'fixed' THEN 'Despesas Fixas' WHEN cat_type = 'variable' THEN 'Despesas Variáveis' ELSE 'Custos Administrativos' END AS label,
      CASE WHEN cat_type = 'fixed' THEN 'fixed' WHEN cat_type = 'variable' THEN 'variable' ELSE 'administrative' END AS type_key,
      json_agg(json_build_object('name', cat_name, 'value', total, 'percentage', CASE WHEN v_total_admin_expenses > 0 THEN ROUND((total / v_total_admin_expenses) * 100, 1) ELSE 0 END)) AS items,
      SUM(total) AS group_total
    FROM expense_cats GROUP BY cat_type
  )
  SELECT COALESCE(json_agg(json_build_object('label', g.label, 'total', g.group_total, 'type', g.type_key, 'items', g.items)),
    json_build_array(
  ...
  ) INTO v_expense_breakdown FROM grouped g;

  -- ========================================================================
  -- 8. TOP PROFIT / LOSS ORDERS
  -- ========================================================================
  SELECT COALESCE(json_agg(row_to_json(t)), '[]'::json) INTO v_top_profit FROM (
    SELECT l.id::text AS id, COALESCE(po.number, '') AS "orderNumber", COALESCE(po.partner_name, l.metadata->>'supplierName', '') AS "partnerName",
      ROUND(l.total_sales_value - l.total_purchase_value - l.total_freight_value, 2) AS profit,
      CASE WHEN l.total_sales_value > 0 THEN ROUND(((l.total_sales_value - l.total_purchase_value - l.total_freight_value) / l.total_sales_value) * 100, 1) ELSE 0 END AS margin
    FROM ops_loadings l LEFT JOIN ops_purchase_orders po ON po.id = l.purchase_order_id
    WHERE l.company_id = p_company_id AND l.status NOT IN ('canceled', 'cancelled') AND l.loading_date >= v_date_from AND l.total_sales_value > 0
    ORDER BY (l.total_sales_value - l.total_purchase_value - l.total_freight_value) DESC LIMIT 5
  ) t;

  SELECT COALESCE(json_agg(row_to_json(t)), '[]'::json) INTO v_top_loss FROM (
    SELECT l.id::text AS id, COALESCE(po.number, '') AS "orderNumber", COALESCE(po.partner_name, l.metadata->>'supplierName', '') AS "partnerName",
      ROUND(l.total_sales_value - l.total_purchase_value - l.total_freight_value, 2) AS profit,
      CASE WHEN l.total_sales_value > 0 THEN ROUND(((l.total_sales_value - l.total_purchase_value - l.total_freight_value) / l.total_sales_value) * 100, 1) ELSE 0 END AS margin
    FROM ops_loadings l LEFT JOIN ops_purchase_orders po ON po.id = l.purchase_order_id
    WHERE l.company_id = p_company_id AND l.status NOT IN ('canceled', 'cancelled') AND l.loading_date >= v_date_from AND l.total_sales_value > 0
    ORDER BY (l.total_sales_value - l.total_purchase_value - l.total_freight_value) ASC LIMIT 5
  ) t;

  -- ========================================================================
  -- 9. BEST / WORST MONTHS
  -- ========================================================================
  SELECT COALESCE(json_agg(x.elem), '[]'::json) INTO v_best_months FROM (
    SELECT elem FROM json_array_elements(COALESCE(v_monthly_history, '[]'::json)) elem
    WHERE (elem->>'netResult')::numeric != 0 ORDER BY (elem->>'netResult')::numeric DESC LIMIT 3
  ) x;

  SELECT COALESCE(json_agg(x.elem), '[]'::json) INTO v_worst_months FROM (
    SELECT elem FROM json_array_elements(COALESCE(v_monthly_history, '[]'::json)) elem
    WHERE (elem->>'netResult')::numeric != 0 ORDER BY (elem->>'netResult')::numeric ASC LIMIT 3
  ) x;
...
SELECT 'MIGRATION_20260526_FIX_PERFORMANCE_RPC_HARVESTS_COMPLETE' AS status;
