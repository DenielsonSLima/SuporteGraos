...
const WatermarkSettings: React.FC<Props> = ({ onBack }) => {
  ...
  useEffect(() => {
    const current = settingsService.getWatermark();
    setWatermarkImage(current.imageUrl);
    setOpacity(current.opacity);
    setOrientation(current.orientation);
  }, []);

  useEffect(() => {
    const unsubscribe = settingsService.onWatermarkChange(next => {
      setWatermarkImage(next.imageUrl);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      await settingsService.updateWatermark({
