...
import { DEFAULT_COMPANY_DATA, useCompany, useUpdateCompany } from '../../../hooks/useCompany';
import { fetchCnpjData } from './company.service';
...
const CompanySettings: React.FC<Props> = ({ onBack }) => {
  ...
  const { data: company, isLoading: isFetching } = useCompany();
  ...
  // Initial State local (fallback while loading)
  const [formData, setFormData] = useState(() => DEFAULT_COMPANY_DATA);

  // Sincronizar dados do hook com o formulário local
  useEffect(() => {
    if (company && !isDirty) {
      setFormData(company);
  ...
  if (isFetching && !company) {
    return (
      <SettingsSubPage
        title="Empresa"
        description="Carregando dados da organização..."
        icon={Building2}
        color="bg-blue-500"
        onBack={onBack}
      >
