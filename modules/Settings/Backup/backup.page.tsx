...
import { useBackupOrchestrator } from './useBackupOrchestrator';
...
const BackupSettings: React.FC<Props> = ({ onBack }) => {
  ...
  const { generateBackupZip, parseRestoreFile, executeRestore } = useBackupOrchestrator();
  ...
  const handleGenerateBackup = async () => {
    ...
    try {
      await generateBackupZip(selectedBackupModules, AVAILABLE_MODULES);
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const foundModules = await parseRestoreFile(file, AVAILABLE_MODULES);
  ...
  const handleExecuteRestore = async () => {
    ...
    setRestoreStage('processing');
    try {
      await executeRestore(uploadedFile, selectedRestoreModules, AVAILABLE_MODULES);
