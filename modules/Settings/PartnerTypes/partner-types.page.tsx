...
import { usePartnerTypes, useAddPartnerType, useUpdatePartnerType, useDeletePartnerType } from '../../../hooks/useClassifications';
...
const PartnerTypesSettings: React.FC<Props> = ({ onBack }) => {
  ...
  const { data: types = [], isLoading } = usePartnerTypes();
  const addPartnerType = useAddPartnerType();
  const updatePartnerType = useUpdatePartnerType();
  const deletePartnerType = useDeletePartnerType();
  ...
  const handleDelete = async (id: string) => {
    ...
    if (type?.isSystem) {
      addToast('error', 'Protegido', 'Este é um registro padrão do sistema e não pode ser excluído.');
      return;
    }
    try {
      await deletePartnerType.mutateAsync(id);
      addToast('success', 'Excluído', 'Tipo de parceiro removido com sucesso.');
    } catch (err: any) {
      addToast('error', 'Erro ao Excluir', err.message ?? 'Não foi possível excluir.');
    }
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        ...
        if (existing) {
          await updatePartnerType.mutateAsync({
            ...existing,
            name:        existing.isSystem ? existing.name : formData.name,
            description: formData.description,
          });
        }
        addToast('success', 'Atualizado', 'Tipo de parceiro atualizado com sucesso.');
      } else {
        await addPartnerType.mutateAsync(formData);
        addToast('success', 'Cadastrado', 'Novo tipo de parceiro adicionado.');
      }
      setViewMode('list');
    } catch (err: any) {
