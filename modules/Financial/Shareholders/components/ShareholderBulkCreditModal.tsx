...
interface Props {
  ...
  onProcessTransaction: (type: 'credit' | 'payment', data: { shareholderId: string, date: string, value: number, description: string, accountId?: string, accountName?: string }) => Promise<void> | void;
}
...
const ShareholderBulkCreditModal: React.FC<Props> = ({ isOpen, onClose, shareholders, onProcessTransaction }) => {
  ...
  const handleCredit = async (shareholderId: string) => {
    const val = parseFloat(values[shareholderId]);
    if (!val || val <= 0) return addToast('warning', 'Valor Inválido');
    try {
      await onProcessTransaction('credit', { shareholderId, date: globalDate, value: val, description: globalDescription });
  ...
  const confirmPayment = async () => {
    ...
    try {
      await onProcessTransaction('payment', { shareholderId: payingShareholderId, date: globalDate, value: val, description: globalDescription, accountId: selectedAccountId, accountName: account?.account_name });
  ...
  return (
    <ModalPortal>
      <div className="fixed inset-0 z-[9999] flex items-start justify-center bg-black/60 backdrop-blur-sm p-0 sm:p-4 animate-in fade-in overflow-y-auto">
        <div className="w-full max-w-4xl bg-white sm:rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 flex flex-col my-0 sm:my-8 min-h-screen sm:min-h-0 sm:max-h-[90vh]">
          ...
          {payingShareholderId && (
              <div className="absolute inset-0 z-20 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center animate-in fade-in p-4">
                  <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95">
                      ...
                      <div className="p-6 space-y-4"><div className="text-center"><p className="text-xs text-slate-500 uppercase font-bold">Valor a Pagar</p><p className="text-3xl font-black text-rose-600 mt-1">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(parseFloat(values[payingShareholderId] || '0'))}</p><p className="text-sm font-bold text-slate-800 mt-2">Para: {shareholders.find(s => s.id === payingShareholderId)?.name}</p></div><div><label className="block text-[10px] font-black text-slate-400 uppercase mb-1">Selecione a Conta de Saída (Saldo Real)</label><div className="relative"><Wallet className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18} /><select value={selectedAccountId} onChange={(e) => setSelectedAccountId(e.target.value)} className="w-full pl-10 pr-4 py-3 border-2 border-rose-100 rounded-xl bg-white text-slate-800 font-bold focus:border-rose-500 outline-none appearance-none"><option value="">Escolha o Banco...</option>{bankAccounts.map(acc => (<option key={acc.id} value={acc.id}>{formatAccountLabel(acc.account_name, acc.owner, acc.balance)}</option>))}</select><ArrowDown className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" size={16} /></div></div><button onClick={confirmPayment} disabled={!selectedAccountId} className="w-full py-3 bg-rose-600 hover:bg-rose-700 disabled:opacity-50 text-white rounded-xl font-black text-sm uppercase shadow-lg transition-all active:scale-95">Efetivar Pagamento</button></div>
                  </div>
              </div>
          )}
        </div>
