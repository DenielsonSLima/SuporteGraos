...
import ShareholderStatementTemplate from '../templates/ShareholderStatementTemplate';
  ...
  const handlePrint = () => {
    ...
    if (printWindow) {
      printWindow.document.write(`
        <html>
          <head>
            <title>Extrato Sócio - ${shareholder.name}</title>
      ...
      <div className="w-full max-w-4xl bg-slate-200 rounded-xl shadow-2xl overflow-hidden flex flex-col h-[90vh]">
        ...
        <div className="flex-1 overflow-y-auto p-8 flex justify-center bg-slate-500/50">
          ...
          >
            ...
            <div ref={contentRef}>
              <ShareholderStatementTemplate shareholder={shareholder} />
            </div>
          </div>
        </div>
