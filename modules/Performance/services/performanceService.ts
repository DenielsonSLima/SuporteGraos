...
export const performanceService = {
  getReport: (monthsBack: number | null): PerformanceReport => {
    ...
    const monthlyHistory = Object.values(monthlyMap)
        .map(m => {
          const qty = m.totalQuantitySc || 1;
          return { 
            ...m, 
            netResult: m.revenue - (m.purchaseCost + m.freightCost + m.otherExpenses),
            avgPurchaseCostSc: m.purchaseCost / qty,
            avgFreightCostSc: m.freightCost / qty,
            avgOtherCostSc: m.otherExpenses / qty,
            avgTotalCostSc: (m.purchaseCost + m.freightCost + m.otherExpenses) / qty
          };
        })
        .sort(sortFn);
    ...
    loadings.forEach(l => {
        ...
        if (!harvestMap[uf]) {
          harvestMap[uf] = { uf, volumeTon: 0, volumeSc: 0, avgPurchasePrice: 0, avgSalesPrice: 0, avgFreightPrice: 0, totalPurchase: 0, totalSales: 0 };
        }
        ...
        harvestMap[uf].avgFreightPrice += l.totalFreightValue;
    });
    ...
    const harvests = Object.values(harvestMap).map(h => ({
        ...
        avgFreightPrice: h.volumeTon > 0 ? h.avgFreightPrice / h.volumeTon : 0
