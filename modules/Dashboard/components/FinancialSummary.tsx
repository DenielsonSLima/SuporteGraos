...
// Helper para ajustar a fonte dinamicamente e evitar quebras de layout em valores longos (milhões/negativos)
const getFontSizeClass = (value: number) => {
  const len = formatMoney(value).length;
  if (len > 18) return 'text-xs sm:text-[10px] md:text-[9px] lg:text-[10px] xl:text-xs';
  if (len > 15) return 'text-sm sm:text-xs md:text-[10px] lg:text-xs xl:text-sm';
  if (len > 12) return 'text-base sm:text-sm md:text-xs lg:text-sm xl:text-base';
  if (len > 9) return 'text-lg sm:text-base md:text-sm lg:text-base xl:text-lg';
  return 'text-xl';
};
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-slate-900 p-6 text-white shadow-lg group">
        ...
        <div className="relative z-10">
          ...
          <div className="flex items-end justify-between">
            <div>
              <h3 className={`font-black tracking-tighter whitespace-nowrap text-white mb-1 ${getFontSizeClass(data.totalBankBalance)}`}>{formatMoney(data.totalBankBalance)}</h3>
      ...
      <div className="relative overflow-hidden rounded-2xl bg-white border border-slate-200 p-6 shadow-sm hover:shadow-md transition-shadow">
        ...
        <h3 className={`font-black tracking-tighter whitespace-nowrap text-slate-800 mb-1 ${getFontSizeClass(data.totalLiabilities)}`}>{formatMoney(data.totalLiabilities)}</h3>
      ...
      <div className="relative overflow-hidden rounded-2xl bg-white border border-slate-200 p-6 shadow-sm hover:shadow-md transition-shadow">
        ...
        <h3 className={`font-black tracking-tighter whitespace-nowrap text-slate-800 mb-1 ${getFontSizeClass(data.pendingSalesReceipts)}`}>{formatMoney(data.pendingSalesReceipts)}</h3>
      ...
      <div className="relative overflow-hidden rounded-2xl bg-white border border-slate-200 p-6 shadow-sm hover:shadow-md transition-shadow group">
        ...
        <h3 className={`font-black tracking-tighter whitespace-nowrap text-blue-700 mb-1 ${getFontSizeClass(data.merchandiseInTransitValue)}`}>{formatMoney(data.merchandiseInTransitValue)}</h3>
      ...
      <div className="relative overflow-hidden rounded-2xl bg-slate-900 p-6 text-white shadow-lg group">
        ...
        <div className="relative z-10">
          ...
          <div className="flex items-end justify-between">
            <div>
              <h3 className={`font-black tracking-tighter whitespace-nowrap text-white mb-1 ${getFontSizeClass(netWorth)}`}>{formatMoney(netWorth)}</h3>
