...
import { ArrowLeft, FileSignature, Handshake, ShoppingBag, DollarSign, Calendar, TrendingUp, ExternalLink } from 'lucide-react';
...
const BrokerDetails: React.FC<Props> = ({ broker, onBack }) => {
  ...
  const relatedOrders = useMemo(() => {
    return purchaseService.getAll().filter(o => 
        o.brokerId === broker.id || 
        o.brokerName?.toLowerCase() === (broker.tradeName || broker.name).toLowerCase()
    );
  }, [broker.id, broker.name, broker.tradeName]);

  const totalCommissionOwed = useMemo(() => {
    return relatedOrders.reduce((acc, order) => {
        // No mundo real buscaríamos os romaneios de cada pedido para saber o volume exato, 
        // aqui usamos o total contratual para o resumo do card.
