...
const AllPartnersPdfModal: React.FC<Props> = ({ isOpen, onClose, searchTerm, category, balances }) => {
  const contentRef = useRef<HTMLDivElement>(null);
  const [isGenerating, setIsGenerating] = useState(false);

  // Busca TODOS os parceiros que batem com o filtro (sem paginação limitante)
  const { data, isLoading } = usePartners({
    searchTerm,
    category,
    page: 1,
    pageSize: 2000 // Suficiente para o relatório geral
  });
      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in">
        <div className="w-full max-w-5xl bg-slate-300 rounded-2xl shadow-2xl overflow-hidden flex flex-col h-[95vh]">
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-10 flex justify-center bg-slate-400/30">
            {isLoading ? (
              <div className="flex flex-col items-center justify-center py-20 bg-white shadow-2xl" style={{ width: '210mm', minHeight: '297mm' }}>
                <Loader2 size={40} className="animate-spin text-emerald-600 mb-4" />
                <p className="text-sm font-black text-slate-500 uppercase tracking-widest">Buscando todos os parceiros...</p>
                <p className="text-[10px] text-slate-400 font-bold mt-2 uppercase">Preparando relatório completo</p>
              </div>
            ) : (
              <div 
