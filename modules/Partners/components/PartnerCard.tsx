...
const PartnerCard: React.FC<Props> = ({ 
  ...
  const cityStateLine = [partner.address?.city, partner.address?.state].filter(Boolean).join(' - ');
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 mb-4 pointer-events-none">
        ...
        {partner.nickname && (
          <p className="text-xs text-blue-600 font-bold italic mt-0.5">"{partner.nickname}"</p>
        )}
