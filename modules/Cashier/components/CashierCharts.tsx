...
const CashierCharts: React.FC<Props> = ({ data }) => {
  ...
  const ChartCard = useMemo(() => React.memo(({ title, chartData }: any) => {
    const sum = chartData.reduce((acc: number, d: any) => acc + (d.value || 0), 0);
    return (
      <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col h-auto min-h-[450px] animate-in fade-in duration-700">
        ...
        <div className="flex-1 space-y-3 px-2 overflow-y-auto max-h-[300px]">
          {chartData.map((entry: any, index: number) => {
            const percentage = sum > 0 ? ((entry.value / sum) * 100).toFixed(1) : '0';
            return (
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
      <ChartCard title="Composição de Ativos & Bens" chartData={incomeData} />
      <ChartCard title="Composição de Passivos & Obrigações" chartData={expenseData} />
    </div>
  );
};
