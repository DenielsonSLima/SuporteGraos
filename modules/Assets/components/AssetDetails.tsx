...
const AssetDetails: React.FC<Props> = ({ asset, onBack, onDelete, onEdit, onRefresh }) => {
  ...
  // Busca histórico financeiro vinculado a este ID de Ativo
  const financialHistory = useMemo(() => {
    return financialActionService.getStandaloneRecords()
      .filter(r => r.assetId === asset.id)
      .sort((a, b) => new Date(a.dueDate).getTime() - new Date(b.dueDate).getTime());
  }, [asset.id, isPayModalOpen, isEditModalOpen, refreshKey]);
  ...
  const handleConfirmPayment = (data: PaymentData) => {
    ...
    financialActionService.processRecord(selectedRecord.id, data);
  ...
  const handleUpdateRecord = (updated: any) => {
    if (!selectedRecord) return;
    financialActionService.addAdminExpense(updated);
  ...
  const handleConfirmSale = (data: any) => {
    ...
    for (let i = 0; i < data.installments; i++) {
        ...
        financialActionService.addAdminExpense({ 
            ...
            description: `Venda Ativo: ${asset.name} (${i+1}/${data.installments})`,
    ...
    addToast('success', 'Venda Registrada', `${data.installments} parcelas geradas no Contas a Receber.`);
    setIsSaleModalOpen(false);
    setRefreshKey(prev => prev + 1); // Força atualização local
    onRefresh(); // Notifica componente pai
  };
                                        ...
                                        return (
                                            <tr key={r.id} className="hover:bg-slate-50/50 transition-colors group">
                                                ...
                                                <td className="px-6 py-4 text-center">
                                                    <span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded shadow-xs border ${
