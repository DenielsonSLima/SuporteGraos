...
const monthlyFreightHistoryReport: ReportModule = {
  metadata: {
    ...
    description: 'Relatório completo de todas as cargas do mês com volumes e quebras.',
    category: 'logistics',
    icon: Calendar,
  ...
  fetchData: ({ startDate, endDate, carrierName }) => {
    let records = loadingService.getAll().filter(l => 
      l.status !== 'canceled' &&
      (!startDate || l.date >= startDate) && 
      (!endDate || l.date <= endDate)
    );
    if (carrierName) {
      records = records.filter(l => l.carrierName === carrierName);
    }
    const totalTon = records.reduce((acc, l) => acc + (l.weightKg / 1000), 0);
    const totalValue = records.reduce((acc, l) => acc + l.totalFreightValue, 0);
    return {
      title: 'Histórico de Fretes do Mês',
