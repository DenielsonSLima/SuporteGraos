...
const PdfDocument: React.FC<{ data: GeneratedReportData }> = ({ data }) => {
    ...
    const summaryItems = [
        ...
        { label: 'Total Pago/Adiantado', value: currency(totalPago), highlight: 'positive' as const },
            ...
            <Page size="A4" orientation="landscape" style={pdfStyles.page}>
                ...
                <View style={pdfStyles.section}>
                    <View style={[pdfStyles.tableHeader, { backgroundColor: '#f1f5f9' }]}>
                        <Text style={[localStyles.th, { width: '10%' }]}>Data</Text>
                        <Text style={[localStyles.th, { width: '25%' }]}>Transportadora</Text>
                        <Text style={[localStyles.th, { width: '22%' }]}>Detalhes da Carga</Text>
                        <Text style={[localStyles.th, { width: '13%', textAlign: 'right' }]}>Valor Frete</Text>
                        <Text style={[localStyles.th, { width: '15%', textAlign: 'right' }]}>Adianta./Pgto</Text>
                        <Text style={[localStyles.th, { width: '15%', textAlign: 'right' }]}>Saldo Pendente</Text>
                    </View>
                        ...
                        <View wrap={false} key={idx} style={{ borderBottomWidth: 1, borderBottomColor: '#e2e8f0', backgroundColor: idx % 2 === 0 ? '#ffffff' : '#f8fafc' }}>
                            <View style={{ flexDirection: 'row' }}>
                                ...
                                <View style={[localStyles.tdContainer, { width: '25%' }]}>
                                    ...
                                    <Text style={localStyles.subText}>Placa: {row.plate}</Text>
                                </View>

                                <View style={[localStyles.tdContainer, { width: '22%' }]}>
                                    <Text style={localStyles.subText}>Orig: {numberFormat(row.weightOrigin)} kg | Desc: {numberFormat(row.weightDest)} kg</Text>
                                    <Text style={localStyles.subText}>Calc. via: {row.baseCalc}</Text>
                                </View>
                                ...
                                <View style={[localStyles.tdContainer, { width: '15%' }]}>
                            ...
                            {row.payments && row.payments.length > 0 && (
                                <View style={{ backgroundColor: '#f8fafc', paddingLeft: '10%', paddingRight: '15%' }}>
                                    {row.payments.map((p: any, pIdx: number) => (
                                        <View key={pIdx} style={localStyles.paymentRow}>
                                            <Text style={localStyles.paymentTextDate}>{p.description} em {date(p.date)}</Text>
