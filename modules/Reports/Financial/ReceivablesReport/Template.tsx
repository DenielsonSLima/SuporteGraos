    ...
    <ReportLayout title={data.title} subtitle={data.subtitle}>
      <table className="w-full border-collapse mb-6">
        <thead>
          <tr className="bg-slate-100 border-b border-slate-300">
            ...
            <th className="py-2 px-2 text-left font-bold uppercase text-slate-700">Cliente / Devedor</th>
            ...
            <th className="py-2 px-2 text-right font-bold uppercase text-slate-700">Saldo a Receber</th>
          </tr>
        </thead>
        <tbody>
          {data.rows.map((row: any, idx: number) => (
            <tr key={idx} className={`border-b border-slate-100 ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}`}>
              ...
              <td className="py-1.5 px-2 text-right font-bold text-emerald-700">{currency(row.balance)}</td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      <div className="flex justify-end border-t-2 border-slate-800 pt-4">
        <div className="text-right">
          ...
          <p className="text-xl font-bold text-emerald-600">{currency(totalPending)}</p>
        </div>
      </div>
    </ReportLayout>
  );
