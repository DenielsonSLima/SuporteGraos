...
const loansReport: ReportModule = {
  ...
  fetchData: () => {
    const loans = reportsCache.getAllLoans();
    const activeLoans = loans.filter(l => l.status === 'active');
    
    // Totals
    const totalTaken = activeLoans.filter(l => l.type === 'taken').reduce((acc, l) => acc + l.remainingValue, 0);
    const totalGranted = activeLoans.filter(l => l.type === 'granted').reduce((acc, l) => acc + l.remainingValue, 0);
    ...
    return {
      ...
      columns: [
        ...
        { header: 'Início', accessor: 'contractDate', format: 'date' },
        { header: 'Vencimento', accessor: 'nextDueDate', format: 'date' },
        { header: 'Valor Original', accessor: 'totalValue', format: 'currency', align: 'right' },
        { header: 'Taxa', accessor: 'rateLabel', align: 'right' },
        { header: 'Saldo Atual', accessor: 'remainingValue', format: 'currency', align: 'right' }
      ],
      rows: loans.map(l => ({
        ...l,
        typeLabel: l.type === 'taken' ? 'Tomado (Passivo)' : 'Concedido (Ativo)',
        rateLabel: `${l.interestRate}% a.m.`
      })),
      summary: [
