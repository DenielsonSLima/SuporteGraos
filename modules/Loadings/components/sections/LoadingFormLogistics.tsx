...
const LoadingFormLogistics: React.FC<Props> = ({
    ...
    fleetPartnerId, onSetFormData, onShowQuickDriverModal
}) => {
    const labelClass = 'text-[10px] font-black text-slate-400 mb-2 uppercase tracking-widest ml-1 block';
    const inputClass = 'w-full rounded-2xl border-2 border-slate-100 bg-white px-5 py-3.5 text-slate-900 font-bold focus:border-blue-500 focus:ring-0 outline-none transition-all placeholder:text-slate-300 text-sm shadow-sm disabled:bg-slate-50 disabled:text-slate-400 disabled:border-slate-100';
    ...
    return (
        <div className="bg-white p-8 rounded-[2rem] border border-slate-200 shadow-sm transition-all hover:shadow-md">
            <h3 className="text-sm font-black text-slate-900 mb-8 flex items-center gap-3 uppercase tracking-tighter italic">
                <div className="p-2 bg-blue-50 text-blue-600 rounded-xl shadow-inner"><Truck size={20} /></div>
                2. Logística & Frota
            </h3>
