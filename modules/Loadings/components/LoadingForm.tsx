...
const LoadingForm: React.FC<Props> = ({ purchaseOrder, onSave, onClose }) => {
  ...
  // ─── Fleet data via hooks ──────────────────────────────────────────────────
  // Resolve o partnerId correto: FOB usa o customerId do pedido de venda
  ...
  // ─── Totals calculados via função pura ─────────────────────────────────────
  ...
  // Sincroniza totals calculados no formData (para enviar no submit)
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onSave({ ...formData, id: generateUUID(), freightAdvances: 0, freightPaid: 0, productPaid: 0 } as Loading);
                ...
                <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="col-span-2 sm:col-span-1">
                      ...
                      <select className={inputClass} value={formData.driverId} onChange={e => { const d = availableDrivers.find(x => x.id === e.target.value); const fallbackPlate = formData.vehiclePlate || availableVehicles[0]?.plate || ''; setFormData({ ...formData, driverId: e.target.value, driverName: d?.name || '', vehiclePlate: fallbackPlate }); }} required>
                        <option value="">Selecione...</option>
                  ...
                  <div className="space-y-5">
                    <div>
                      ...
                      <div className="relative">
                        <input
                          type="number"
                          className="w-full bg-white border-2 border-slate-300 rounded-xl px-4 py-3 text-2xl font-black text-slate-900 focus:border-emerald-500 focus:outline-none transition-all"
                      ...
                      <div className="flex items-center gap-4">
                        <div className="relative flex-1">
                          <input
                            ...
                            className="w-full bg-white border-2 border-slate-300 rounded-lg px-3 py-2 text-sm font-bold text-slate-900 focus:border-emerald-500 outline-none"
        ...
        <div className="bg-white border-t border-slate-200 px-8 py-6 shrink-0 flex justify-between items-center shadow-[0_-10px_20px_rgba(0,0,0,0.02)]">
          ...
          <div className="flex gap-4 w-full sm:w-auto">
            ...
            <button
              ...
              className="flex-1 sm:flex-none px-12 py-3.5 rounded-xl bg-slate-950 text-white font-black shadow-xl shadow-slate-900/20 hover:bg-slate-800 flex items-center justify-center gap-2 uppercase text-xs tracking-widest active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isSubmitting ? 'Salvando...' : <><Save size={18} /> Confirmar e Salvar</>}
            </button>
          </div>
        </div>
      </div>
