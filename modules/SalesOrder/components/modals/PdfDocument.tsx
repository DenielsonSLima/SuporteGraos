...
const PdfDocument: React.FC<Props> = ({ order, loadings, variant }) => {
  ...
  const stats = useMemo(() => {
    ...
    const totalLoadedValue = activeLoadings.reduce((acc, l) => acc + l.totalSalesValue, 0);
    ...
    const debitedExpenses: typeof expenses = [];
  ...
  const isCustomerVariant = variant === 'producer';
  const title = isCustomerVariant ? 'EXTRATO CONSOLIDADO - CLIENTE' : 'AUDITORIA INTERNA - VENDA';
    ...
    <Document>
      <Page size="A4" style={styles.page} orientation={variant === 'internal' ? 'landscape' : 'portrait'}>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.headerRight}>
            <View style={styles.titleBox}>
              ...
              <Text style={styles.titleMain}>ORDEM DE VENDA</Text>
        ...
        <View style={styles.dashboard}>
          ...
          <View style={styles.dashCard}>
            <Text style={[styles.dashLabel, { color: '#059669' }]}>Total Recebido</Text>
            <Text style={[styles.dashValue, { color: '#047857' }]}>{currency(stats.totalPaid)}</Text>
            <View style={[styles.dashBar, { backgroundColor: '#10b981' }]} />
          </View>

          <View style={styles.dashCard}>
            <Text style={[styles.dashLabel, { color: '#d97706' }]}>Deduções/Taxas</Text>
          ...
          <View style={[
            ...
            <Text style={styles.balanceStatus}>
              {stats.balance <= 0.05 ? 'Totalmente Recebido' : 'Valor Pendente'}
            </Text>
          </View>
        </View>
        ...
        {stats.activeLoadings.length > 0 && (
          <View style={styles.section}>
            ...
            <View style={styles.table}>
              ...
              {stats.activeLoadings.map((loading, idx) => (
                <View key={loading.id} style={styles.tableRow}>
                  ...
                  <Text style={[styles.tableCell, { width: '20%', textAlign: 'right' }]}>{currency(loading.salesPrice)}</Text>
                  <Text style={[styles.tableCellBold, { width: '25%', textAlign: 'right' }]}>{currency(loading.totalSalesValue)}</Text>
                </View>
              ))}
            </View>
            ...
            <View style={styles.summary}>
              ...
              <View style={styles.summaryRow}>
                <Text style={styles.summaryLabel}>Valor Total Vendido:</Text>
        ...
        {/* Recebimentos */}
        {stats.payments.length > 0 && isCustomerVariant && (
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>RECEBIMENTOS E ADIANTAMENTOS</Text>
            <View style={styles.table}>
              <View style={styles.tableHeader}>
                <Text style={[styles.tableCellBold, { width: '15%' }]}>Data</Text>
        ...
        {/* Despesas Debitadas */}
        {stats.debitedExpenses.length > 0 && isCustomerVariant && (
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>DEDUÇÕES DO CLIENTE</Text>
            <View style={styles.table}>
              <View style={styles.tableHeader}>
                <Text style={[styles.tableCellBold, { width: '15%' }]}>Data</Text>
