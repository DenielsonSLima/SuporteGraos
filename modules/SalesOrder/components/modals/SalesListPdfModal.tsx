...
import SalesListPdfDocument from './SalesListPdfDocument';
...
const SalesListPdfModal: React.FC<Props> = ({ 
  isOpen, 
  onClose, 
  orders,
  searchTerm,
  ...
  // ✅ Filtros aplicados para exibir no subtítulo
  ...
  // ✅ Chave de re-render baseada nos pedidos
    ...
    const generatePreview = async () => {
      if (isOpen && isLoaded && orders.length > 0) {
        try {
          ...
          console.error("Erro ao gerar preview do PDF da lista de vendas:", err);
          if (isMounted) {
            setError("Falha na geração do PDF consolidado de vendas. Tente recarregar a página.");
          }
        }
      }
    };
  ...
  const handleDownloadPdf = async () => {
    ...
    const filename = `Relatorio_Pedidos_Venda_${formattedDate}.pdf`;
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="font-black text-lg">
                  Relatório de Pedidos de Venda (Consolidado)
                </h3>
          ...
          <div className="flex-1 overflow-auto p-8 flex justify-center bg-slate-400/20">
            {orders.length === 0 ? (
              <div className="flex flex-col items-center justify-center gap-4 bg-white w-full h-full rounded-lg shadow-inner text-center p-10">
                ...
                <p className="text-slate-500 text-sm max-w-sm">Não há pedidos de venda para listar com os filtros selecionados no momento.</p>
              </div>
            ) : !isLoaded ? (
...
export default SalesListPdfModal;
