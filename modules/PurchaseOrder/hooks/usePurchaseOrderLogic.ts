...
export const usePurchaseOrderLogic = (initialOrder: PurchaseOrder, onFinalizeCallback: () => void) => {
  ...
  const refreshLoadings = (id?: string) => {
    ...
    const list = LoadingCache.getByPurchaseOrder(orderId);
    setLoadings([...list]); 
  ...
  const actions = {
    ...
    handleAddExpense: (data: any) => {
        ...
        // 2. Cria movimentação financeira real (pagamento)
    ...
    // Added handleAddCommission for managing broker commissions
    handleAddCommission: (data: any) => {
        ...
        // ✅ Criar automaticamente um payable da comissão
