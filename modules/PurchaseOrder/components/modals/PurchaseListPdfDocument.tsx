...
const PurchaseListPdfDocument: React.FC<Props> = ({ 
  orders = [], 
  title = 'Relatório de Pedidos de Compra',
  subtitle = 'Listagem consolidada de contratos de compra de grãos'
}) => {
  ...
  const getStatusLabel = (status: string) => {
    switch (status) {
      ...
      case 'transport': return 'Em Trânsito';
  ...
  // ✅ Pré-calcula os valores consolidados
  const totals = useMemo(() => {
    return orders.reduce((acc, order) => {
      const contractQty = order.items?.reduce((sum, item) => sum + (Number(item.quantity) || 0), 0) || 0;
      const loadedQty = Number(order.totalSc) || 0;
      const totalValue = Number(order.totalPurchaseValCalc) || Number(order.totalValue) || 0;
  ...
  const columns: PdfTableColumn[] = [
    ...
    },
    {
      header: 'Produto',
      width: '10%',
      render: (row) => row.items?.[0]?.productName || 'N/D',
      align: 'left'
    },
    {
      header: 'Contrato',
      width: '9%',
      render: (row) => {
        const qty = row.items?.reduce((sum: number, item: any) => sum + (Number(item.quantity) || 0), 0) || 0;
        const unit = row.items?.[0]?.unit || 'SC';
    ...
    },
    {
      header: 'Carregado',
      width: '9%',
      render: (row) => `${num(Number(row.totalSc) || 0, 0)} SC`,
      align: 'right'
    },
    {
      header: 'Valor Total',
      width: '10%',
      render: (row) => currency(Number(row.totalPurchaseValCalc) || Number(row.totalValue) || 0),
      align: 'right'
    },
    {
      header: 'Liquidado',
    ...
    },
    {
      ...
      width: '10%',
      render: (row) => {
        const total = Number(row.totalPurchaseValCalc) || Number(row.totalValue) || 0;
  ...
  return (
    <Document>
      <Page size="A4" orientation="landscape" style={pdfStyles.page}>
        ...
        <PdfSummary 
          items={[
            { label: 'Qtd. Total Contratada', value: `${num(totals.contractQty, 0)} SC/Unidades` },
            { label: 'Qtd. Total Carregada', value: `${num(totals.loadedQty, 0)} SC` },
            { label: 'Valor Total Contratos', value: currency(totals.totalValue) },
            { label: 'Total Liquidado/Pago', value: currency(totals.paidValue), highlight: 'success' },
            { label: 'Saldo Total em Aberto', value: currency(totals.pendingValue), isTotal: true, highlight: 'danger' }
          ]}
        />
...
export default PurchaseListPdfDocument;
