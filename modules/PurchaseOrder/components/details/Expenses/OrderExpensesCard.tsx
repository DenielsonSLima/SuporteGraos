...
import { Receipt, Plus, Calendar, Wallet, CheckSquare, Pencil, Trash2, Landmark, Info, Square } from 'lucide-react';
...
const OrderExpensesCard: React.FC<Props> = ({ transactions, onAddExpense, onEditTx, onDeleteTx }) => {
  ...
  const totalDeducted = expenses.filter(t => t.deductFromPartner).reduce((acc, t) => acc + t.value, 0);
  const totalCompanyCost = expenses.filter(t => !t.deductFromPartner).reduce((acc, t) => acc + t.value, 0);
      ...
      <div className="p-6 flex-1 flex flex-col">
        ...
        <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-50 gap-4">
          ...
          <div className="text-right">
             ...
             <span className="text-sm font-bold text-slate-700">{currency(totalDeducted)}</span>
          </div>
          <div className="text-right">
             <span className="text-[9px] font-black uppercase text-slate-500 block mb-0.5">Custo Empresa</span>
             <span className="text-sm font-bold text-slate-700">{currency(totalCompanyCost)}</span>
          </div>
        </div>
